...
;; Board is (listof Val|false), 81 elements long
;; interp.
;;  9x9 array of squares, where each square has a row and a column number
;;  (r, c) and position in a board.

;; Post is Natural [0, 80]
;; interp.
...
;;  There are 27 of these for the 9 rows, 9 columns and 9 boxes.
...
;; produce a solution for board, or false if unsolvable
...
(define (solved? bd)
  (cond [(empty? bd) true]
        [else
         (if (not (integer? (first bd)))
             false
             (solved? (rest bd)))]))

;; Board -> (ListOf Board)
;; Gives the list of valid boards
;; Finds first empty square, fills it with Natural[1, 9], keeps only valid boards

(define (next-boards bd)
  (keep-only-valid (fill-with-numbers (find-blank bd) bd)))

;; Board -> Position
;; the position of the first blank square
...
;; 9 boards, with blank filled with Natural[1, 9]

(define (fill-with-numbers p bd)
  (local [(define (build-one n)
            (fill-square bd p (+ n 1)))]
    (build-list 9 build-one)))

;; (listof Board) -> (listof Board)
;; Keeps only the valid boards
...
;; Gives the value at a given position on the board
...
;; Board Pos Val -> Board
;; Gives a new board with Val at a given position
(check-expect (fill-square BD1 (r-c->pos 0 0) 1)
              (cons 1 (rest BD1)))

(define (fill-square bd p v)
  (append (take bd p)
          (list v)
          (drop bd (add1 p))))
